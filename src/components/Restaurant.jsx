...
function Restaurant() {
  ...
  const filteredRestaurants = safeRestaurants.filter((r) => {
    ...
    const restaurantLocation = r.location || '';
    const restaurantType = r.type || '';
    
    const matchesSearch =
      restaurantName.toLowerCase().includes(searchQuery.toLowerCase()) ||
      restaurantLocation.toLowerCase().includes(searchQuery.toLowerCase());
    const matchesType =
      filterType === "all" || restaurantType.toLowerCase() === filterType.toLowerCase();
  ...
  const handleViewOnMap = (restaurant) => {
    if (restaurant.map_url) {
      window.open(restaurant.map_url, '_blank');
    } else {
      alert('Map location not available for this restaurant.');
    }
  };
          ...
          {filteredRestaurants.map((r) => (
            ...
            >
              ...
              <div className="relative h-64 overflow-hidden">
                <img
                  src={r.image_url || "https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?w=800&h=600&fit=crop"}
                  alt={r.name}
              ...
              <div className="p-6">
                ...
                <button 
                  ...
                  className="w-full bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white font-bold py-4 px-6 rounded-xl transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105 flex items-center justify-center gap-2"
                >
